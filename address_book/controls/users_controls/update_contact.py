...
def update_contact(address_book):
    ...
    if entry_index == 1:
        update_contact_name(contact_index, address_book)
    elif entry_index == 2:
        update_contact_second_name(contact_index, address_book)
    elif entry_index == 3:
        update_contact_last_name(contact_index, address_book)
    elif entry_index == 4:
        ...
        if action == 1:
            update_contact_phone_add_new(contact_index, address_book)
        elif action == 2:
            input_message = "Which phone you want to edit?\n>>> "
            phone_index = validate_the_entered_index(contact_index, "phone", input_message, address_book)
            update_contact_phone_edit(contact_index, phone_index)
        elif action == 3:
            input_message = "Which phone you want to delete?\n>>> "
            phone_index = validate_the_entered_index(contact_index, "phone", input_message, address_book)
            update_contact_phone_delete(contact_index, phone_index)
    elif entry_index == 5:
        if hasattr(address_book[contact_index], "email"):
            menu = "1. ADD NEW EMAIL\n2. UPDATE EXISTING\n3. DELETE EMAIL "
            action = validate_the_menu_action_loop(menu, menu_message_for_user)
        else:
            action = 1
        if action == 1:
            update_contact_email_add_new(contact_index, address_book)
        elif action == 2:
            if len(address_book[contact_index].email) > 0:
                input_message = "Which email you want to edit?\n>>> "
                email_index = validate_the_entered_index(contact_index, "email", input_message, address_book)
                update_contact_email_edit(contact_index, email_index, address_book)
            else:
            ...
            if len(address_book[contact_index].email) > 0:
                ...
                email_index = validate_the_entered_index(contact_index, "email", input_message, address_book)
                update_contact_email_delete(contact_index, email_index, address_book)
            else:
                print("There is no email to delete for this contact")

    elif entry_index == 6:
        if len(address_book[contact_index].social_account) == 1:
        ...
        if action == 1:
            ...
            update_social_account(contact_index, g_social_network_list[add_social_network_index], address_book)
        elif action == 2:
            input_message = "Please enter the social network name you would like to edit.\n>>> "
            key = validate_social_network(contact_index, input_message, address_book)
            update_social_account(contact_index, key, address_book)
        elif action == 3:
            input_message = "Please enter the social network name you would like to delete.\n>>> "
            key = validate_social_network(contact_index, input_message, address_book)
            update_social_account_delete(contact_index, key, address_book)
    elif entry_index == 7:
        if hasattr(address_book[contact_index], "address_street"):
            menu = "1. UPDATE CITY\n2. UPDATE STREET\n3. UPDATE ADDRESS\n4. DELETE STREET"
            action = validate_the_menu_action_loop(menu, menu_message_for_user)
        else:
            menu = "1. UPDATE CITY\n2. UPDATE STREET\n3. UPDATE ADDRESS"
            action = validate_the_menu_action_loop(menu, menu_message_for_user)
        if action == 1:
            update_contact_city(contact_index, address_book)
        elif action == 2:
            update_contact_street(contact_index, address_book)
        elif action == 3:
            update_contact_city(contact_index, address_book)
            update_contact_street(contact_index, address_book)
        elif action == 4:
            update_contact_street_delete(contact_index, address_book)